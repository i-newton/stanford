...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for in_dim, out_dim,l_num in zip(input, output, range(1, self.num_layers + 1)):
            ...
            if self.use_batchnorm and l_num < self.num_layers:
                self.params['gamma' + l_num_str] = None
                self.params['beta' + l_num_str] = None
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def loss(self, X, y=None):
        ...
        for l_num in range(1, self.num_layers):
            ...
            b_label = 'b' + str(l_num)
            if self.use_batchnorm:
              g_name = 'gamma' + str(l_num)
              b_name = 'beta' + str(l_num)
              if self.params.get(g_name) is not None:
                  self.bn_params[l_num - 1]['gamma'] = self.params[g_name]
              if self.params.get(b_name) is not None:
                  self.bn_params[l_num - 1]['beta'] = self.params[b_name]
              input, cache =self.forward(input, self.params[W_label], self.params[b_label], self.bn_params[l_num - 1])
              self.params[g_name] = self.bn_params[l_num - 1]['gamma']
              self.params[b_name] = self.bn_params[l_num - 1]['beta']
            else: 
        ...
        for l_num in range(self.num_layers - 1, 0, -1):
            ...
            b_name = 'b'+str(l_num)
            if self.use_batchnorm:
                grad_out, dwi, dbi, dgamma, dbeta = self.backward(grad_out, caches[l_num - 1])
                grads['gamma' + str(l_num)] = dgamma
                grads['beta' + str(l_num)] = dbeta
            else:
